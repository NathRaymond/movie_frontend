                ...
                <div class="modal-body">
                    <div class="card-body">
                        <form method="POST" action="{{ route('update-users') }}" onsubmit="$('.preloader').show()">
                            ...
                            <div class="form-group">
                                <label class="form-label">Phone Number:</label>
                                <input type="number" id="uPhone" name="phone_number"class="form-control" required>
                            </div>
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                <button type="submit" class="btn btn-primary">Save <span
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '#edit-user', function() {
                // alert("Nathaniel")
                var id = $(this).data('id');
                $.get('{{ route('getuser') }}?id=' + id, function(data) {
                    // console.log(data[2]);
                    $('#uName').val(data.name);
                ...
                try {
                    $.get('{{ route('delete_user') }}?id=' + user_id,
                        function(data, status) {
                            ...
                            console.table(data);
                            if (status === "success") {
                                let alert = swal("User deleted successfully.");
